...
from mmdetection.mmdet.models.utils.pose_utils import reconstruction_error, reconstruction_error_vis, vectorize_distance
from mmdetection.mmdet.core.utils import AverageMeter
from mmdetection.mmdet.models.utils.smpl_utils import perspective_projection
from mmdetection.mmdet.models.utils.smpl.renderer import Renderer
from mmdetection.mmdet.models.utils.smpl.smpl import SMPL, JointMapper
from mmdetection.mmdet.models.utils.smpl.viz import draw_skeleton, get_bv_verts, plot_pose_H36M
import cv2
...
def compute_PCK(gt_kpts, pred_kpts, threshold=150):
    PCK_THRESHOLD = threshold
    sample_num = len(gt_kpts)
    total = 0
    true_positive = 0
    for n in range(sample_num):
        pred = pred_kpts[n]
        gt = gt_kpts[n]
        per_joint_error = np.sqrt(np.sum(np.power(pred - gt, 2), 1)) * 1000
...
def compute_AUC(gts, preds, auc_min, auc_max):
    # This code is adapted from 3dpw-eval.py
    thresholds = np.linspace(auc_min, auc_max)
    pck_list = []
    auc_range = np.arange(auc_min, auc_max)
    for threshold in auc_range:
        pck_list.append(compute_PCK(gts, preds, threshold))
    ...
    auc = compute_auc(auc_range / auc_range.max(), pck_list)
...
class H36MEvalHandler(EvalHandler):
    ...
    def log(self):
        ...
        return f'p1: {self.p1_meter.avg:.2f}mm, p2: {self.p2_meter.avg:.2f}mm'
    
    def finalize(self):
        self.log()
        results = f'p1: {self.p1_meter.avg:.2f}mm, p2: {self.p2_meter.avg:.2f}mm'
        return results
...
class PanopticEvalHandler(EvalHandler):
    ...
    def log(self):
        ...
        return f'p1: {self.p1_meter.avg:.2f}mm, coll_cnt: {self.coll_cnt} coll: {self.collision_meter.avg} L'

    def finalize(self):
        self.log()
        results = f'p1: {self.p1_meter.avg:.2f}mm, coll_cnt: {self.coll_cnt} coll: {self.collision_meter.avg} L'
        return results
...
class MuPoTSEvalHandler(EvalHandler):
    ...
    def handle(self, data_batch, pred_results, use_gt=False):
        ...
        pred_keypoints_2d_smpl = perspective_projection(
            pred_keypoints_3d_smpl, rotation=rotation_Is, translation=pred_translation, 
            focal_length=self.FOCAL_LENGTH, camera_center=img_size/2)
        if self.viz_dir:
            img = img.clone() * torch.tensor([0.229, 0.224, 0.225]).view(3, 1, 1) + torch.tensor(
                [0.485, 0.456, 0.406]).view(3, 1, 1)
            img_cv = img.clone().numpy()
            ...
            img_cv = (img_cv / 255.)
            fname = osp.basename(data_batch['img_meta'].data[0]['file_name'])
            plt.imsave(osp.join(self.viz_dir, fname), img_cv)

        scale_factor = data_batch['img_meta'].data[0]['scale_factor']
        raw_kpts2d = pred_keypoints_2d_smpl / scale_factor
        self.result_list_2d.append(raw_kpts2d[:, self.h36m_to_MPI])
        return {'file_name': data_batch['img_meta'].data[0]['file_name'], 'pred_kpts3d': pred_keypoints_3d_smpl}
    ...
    def finalize(self):
        ...
        self.log()
        results = f'coll_cnt: {self.coll_cnt} coll {self.collision_meter.avg} L'
        return results


class PW3DEvalHandler(EvalHandler):
    def __init__(self, JOINT_REGRESSOR_H36M='data/J_regressor_h36m.npy', **kwargs):
        super().__init__(**kwargs)
        self.J_regressor = torch.from_numpy(np.load(JOINT_REGRESSOR_H36M)).float()
        self.p1_meter = AverageMeter('P1', ':.2f')
        self.p2_meter = AverageMeter('P2', ':.2f')
        self.mpvpe = AverageMeter('MPVPE', ':.2f')
        self.pck = AverageMeter('PCK', ':.2f')
        self.auc = AverageMeter('AUC', ':.2f')
        self.stats = list()
        self.mismatch_cnt = 0
        self.PCK_THRESH = 50.0
        self.AUC_MIN = 0.0
        self.AUC_MAX = 200.0
        # Initialize SMPL model
        openpose_joints = [24, 12, 17, 19, 21, 16, 18, 20, 0, 2, 5, 8, 1, 4,
    ...
    def handle(self, data_batch, pred_results, use_gt=False):
        ...
        pred_translation = pred_results['pred_translation'].cpu()
        bboxes = pred_results['bboxes'][:, :4]
        gt_vertices = data_batch['gt_vertices'].data[0][0].clone().cpu()
        gt_keypoints_3d = data_batch['gt_kpts3d'].data[0][0].clone()
        gt_pelvis_smpl = gt_keypoints_3d[:, [14], :-1].clone()
        visible_kpts = gt_keypoints_3d[:, J24_TO_J14, -1].clone()
        origin_gt_kpts3d = data_batch['gt_kpts3d'].data[0][0].clone().cpu()
        origin_gt_kpts3d = origin_gt_kpts3d[:, J24_TO_J14]
        origin_gt_kpts3d[:, :, :-1] -= gt_pelvis_smpl
        gt_keypoints_3d = gt_keypoints_3d[:, J24_TO_J14, :-1].clone()
        gt_keypoints_3d = gt_keypoints_3d - gt_pelvis_smpl
        gt_vertices = gt_vertices - gt_pelvis_smpl

        J_regressor_batch = self.J_regressor[None, :].expand(pred_vertices.shape[0], -1, -1).to(
            pred_vertices.device)
        pred_vertices = pred_vertices.clone().cpu()
        # Get 14 predicted joints from the SMPL meshz
        pred_keypoints_3d_smpl = torch.matmul(J_regressor_batch, pred_vertices)
        pred_pelvis_smpl = pred_keypoints_3d_smpl[:, [0], :].clone()
        pred_keypoints_3d_smpl = pred_keypoints_3d_smpl[:, H36M_TO_J14, :]
        pred_keypoints_3d_smpl = pred_keypoints_3d_smpl - pred_pelvis_smpl
        ...
        if is_mismatch:
            self.mismatch_cnt += 1
        selected_prediction = pred_keypoints_3d_smpl[paired_idxs]
        selected_vertices = pred_vertices[paired_idxs]
        selected_pelvis = pred_pelvis_smpl[paired_idxs]
        ...
        # PAMPJPE
        r_error_smpl = reconstruction_error(selected_prediction.cpu().numpy(), 
                                            gt_keypoints_3d.cpu().numpy())
        r_error = float(r_error_smpl.mean() * 1000)
        self.p2_meter.update(r_error)

        # MPVPE
        selected_vertices = selected_vertices - selected_pelvis
        error_vertices = (torch.sqrt(((selected_vertices - gt_vertices) ** 2).sum(dim=-1)))
        mpvpe = float(error_vertices.mean() * 1000)
        self.mpvpe.update(mpvpe)
        
        # PCK and AUC
        pck = compute_PCK(gt_keypoints_3d.cpu().numpy(), selected_prediction.cpu().numpy(), self.PCK_THRESH)
        self.pck.update(pck)
        auc = compute_AUC(gt_keypoints_3d.cpu().numpy(), selected_prediction.cpu().numpy(), self.AUC_MIN, self.AUC_MAX)
        self.auc.update(auc)
    ...
    def log(self):
        self.writer(
            f'p1: {self.p1_meter.avg:.2f}mm, p2: {self.p2_meter.avg:.2f}mm, mpvpe: {self.mpvpe.avg:.2f}mm, pck: {self.pck.avg:.2f}, auc: {self.auc.avg:.2f}')

    def finalize(self):
        self.log()
        results = f'p1: {self.p1_meter.avg:.2f}mm, p2: {self.p2_meter.avg:.2f}mm, mpvpe: {self.mpvpe.avg:.2f}mm, pck: {self.pck.avg:.2f}, auc: {self.auc.avg:.2f}'
        return results


class PoseTrackEvalHandler(EvalHandler):
    def __init__(self, JOINT_REGRESSOR_H36M='data/J_regressor_h36m.npy', **kwargs):
        super().__init__(**kwargs)
        self.J_regressor = torch.from_numpy(np.load(JOINT_REGRESSOR_H36M)).float()
        self.p1_meter = AverageMeter('P1', ':.2f')
    ...
    def log(self):
        self.writer(
            f'coll_cnt: {self.coll_cnt} coll: {self.collision_meter.avg} L')
