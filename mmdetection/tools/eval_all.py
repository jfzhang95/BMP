...
"""
python3 tools/eval_all.py configs/bmp/finetune.py --ckpt ./work_dirs/finetune/latest.pth
"""
...
def main():
    ...
    ckpt_name = args.ckpt.split('/')[-1][:-4]
    txt_name = 'eval_results/{}_results.txt'.format(ckpt_name)
    if os.path.exists(txt_name):
        os.remove(txt_name)
    txt_file = open(txt_name, 'a')
    for dataset_eval in ['haggling', 'pizza', 'mafia', 'ultimatum', 'pw3d', 'mupots']:
        dataset_cfg = eval_dataset_mapper[dataset_eval]
        if dataset_eval == 'pw3d':
        ...
        shuffle = False if dataset_eval in stable_list else True
        ...
        dump_dir = os.path.join(cfg.work_dir, f'eval_{dataset_eval}')
        os.makedirs(dump_dir, exist_ok=True)
        if args.viz_dir:
            os.makedirs(args.viz_dir, exist_ok=True)
        eval_handler = eval_handler_mapper[dataset_eval](writer=tqdm.write, viz_dir=args.viz_dir,
        ...
        results = eval_handler.finalize()
        txt_file.write('{}: '.format(dataset_eval)+results)
    txt_file.close()
