...
class Runner(object):
    ...
    def train(self, data_loader, **kwargs):
        ...
        for i, data_batch in enumerate(data_loader):
            ...
            outputs = self.batch_processor(
                self.model, data_batch, mode=self.mode, **kwargs)
            ...
            # Save checkpoints when time is up.
            # It's easy to do, just a break statement is enough, but it will make if we don't care about adjusting the
            # learning rate.
            # Validate inside epoch
    ...
    def run(self, data_loaders, workflow, max_epochs, **kwargs):
        ...
        if self.exit_code is not None:
            print(f'exit with code: {self.exit_code}')
            sys.stdout.flush()
            sys.exit(self.exit_code)
        else:
            print(f'exit with code: 3 after finish')
            sys.stdout.flush()
            sys.exit(3)
