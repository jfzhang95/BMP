...
class TextLoggerHook(LoggerHook):
    ...
    def _get_max_cache(self, runner):
    ...
    def _log_info(self, log_dict, runner):
        if runner.mode == 'train':
            ...
            if 'time' in log_dict.keys():
                ...
                log_str += 'cache: {}, '.format(log_dict['cache'])
        else:
            print()
    ...
    def log(self, runner):
        ...
        if mode == 'train':
            ...
            if torch.cuda.is_available():
                ...
                log_dict['cache'] = self._get_max_cache(runner)
        for name, val in runner.log_buffer.output.items():
            if name in ['time', 'data_time'] or name.startswith('img$'):
                continue
            log_dict[name] = val
